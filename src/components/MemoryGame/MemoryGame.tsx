...
function MemoryGame() {
  const [randArr, setRandArr] = useState<Array<string>>([]);
  ...
  const [compareCards, setCompareCards] = useState<Array<string>>([]);
  const [logId, setLogId] = useState<Array<string>>([]);
  ...
  function resetAll() {
    ...
    updatedm = 0;
    updateds = 0;
    setTimeout(() => setStime(0), 100);
    setTimeout(() => setMtime(0), 100);
    setCompareCards([]);
    setLogId([]);
    setTimerOn(false);
    setStarted(false);
  ...
  function disableClickAll() {
    ...
    if (!arr.includes("id10")) {
      setClCard10("card-noclick");
    }
    if (!arr.includes("id11")) {
      setClCard11("card-noclick");
    }
    if (!arr.includes("id12")) {
      setClCard12("card-noclick");
    }
    if (!arr.includes("id13")) {
      setClCard13("card-noclick");
    }
    if (!arr.includes("id14")) {
      setClCard14("card-noclick");
    }
    if (!arr.includes("id15")) {
      setClCard15("card-noclick");
    }
    if (!arr.includes("id16")) {
      setClCard16("card-noclick");
    }
  }
  ...
  function disableClickId(id: string) {
    switch (id) {
      case "id1":
        setClCard1("disabled");
        break;
  ...
  useEffect(() => {
    let interval: NodeJS.Timer = setTimeout(function () {
      /* snip */
    }, 1);
